"""Hybrid analyst combining programmatic and agent-based analysis."""
...
class HybridAnalyst:
    ...
    def analyze_dataset(self, file_path: str, dataset_name: str = None) -> Dict[str, Any]:
        """Analyze dataset using hybrid approach."""
        
        log_stage("HYBRID ANALYSIS", f"Starting hybrid analysis for {file_path}", "🔬")
        ...
        try:
            ...
            readable_logger.start_loading("🤖 Running Hybrid Analysis...")
            log_process_start("Hybrid Analysis", f"Processing {dataset_name} with 5 agents")
            
            result = crew.kickoff()
            
            log_process_end("Hybrid Analysis", "Success", f"Completed analysis for {dataset_name}")
            ...
            log_success("Hybrid Analysis", f"Analysis complete for {dataset_name}")
            
            return analysis_result
            
        except Exception as e:
            log_error("Hybrid Analysis Error", str(e))
            raise e
    ...
    def _save_analysis_report(self, analysis_result: Dict[str, Any], dataset_name: str):
        ...
        report_file = self.output_dir / f"hybrid_analysis_report_{dataset_name}_{timestamp}.json"
        
        with open(report_file, 'w') as f:
            json.dump(analysis_result, f, indent=2, default=str)
        
        log_event("Report Saved", f"Hybrid analysis report saved: {report_file.name}", "📄")


def main():
    """Test the hybrid analyst."""
    analyst = HybridAnalyst()
    
    # Test with any dataset in data folder
    data_files = list(Path("data").glob("*.csv"))
    if data_files:
        result = analyst.analyze_dataset(str(data_files[0]))
